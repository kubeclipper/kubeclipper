...
if [ "${GENS}" = "all" ] || grep -qw "openapi" <<<"${GENS}"; then
  ...
  "${GOPATH}/bin/openapi-gen" \
           --input-dirs "$(codegen::join, "${OPENAPI_EXTRA_PACKAGES[@]}")" \
           --input-dirs "k8s.io/apimachinery/pkg/apis/meta/v1,k8s.io/apimachinery/pkg/runtime,k8s.io/apimachinery/pkg/version,k8s.io/api/rbac/v1,${EXT_FQ_APIS[@]}" \
