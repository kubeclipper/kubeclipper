...
func (f *oidcProviderFactory) Create(options oauth.DynamicOptions) (identityprovider.OAuthProvider, error) {
	var idp oidcProvider
	if err := mapstructure.Decode(options, &idp); err != nil {
		return nil, err
	}
	// dynamically discover
	if idp.Issuer != "" {
		ctx := context.TODO()
		if idp.InsecureSkipVerify {
			client := &http.Client{
		...
		idp.Endpoint.AuthURL, _ = providerJSON["authorization_endpoint"].(string)
		idp.Endpoint.TokenURL, _ = providerJSON["token_endpoint"].(string)
		idp.Endpoint.UserInfoURL, _ = providerJSON["userinfo_endpoint"].(string)
		idp.Endpoint.JWKSURL, _ = providerJSON["jwks_uri"].(string)
		idp.Endpoint.EndSessionURL, _ = providerJSON["end_session_endpoint"].(string)
		idp.Provider = provider
		idp.Verifier = provider.Verifier(&oidc.Config{
			// TODO: support HS256
			ClientID: idp.ClientID,
		})
		options["endpoint"] = oauth.DynamicOptions{
			"authURL":       idp.Endpoint.AuthURL,
			"tokenURL":      idp.Endpoint.TokenURL,
			"userInfoURL":   idp.Endpoint.UserInfoURL,
			"jwksURL":       idp.Endpoint.JWKSURL,
			"endSessionURL": idp.Endpoint.EndSessionURL,
		}
	}
	scopes := []string{oidc.ScopeOpenID}
	if !sliceutil.HasString(idp.Scopes, oidc.ScopeOpenID) {
		scopes = append(scopes, idp.Scopes...)
	}
	idp.Scopes = scopes
	idp.OAuth2Config = &oauth2.Config{
		ClientID:     idp.ClientID,
...
func (o *oidcProvider) IdentityExchange(req *http.Request) (identityprovider.Identity, error) {
