...
var log = logger.WithName("source").WithName("EventHandler")
...
func (e EventHandler) OnAdd(obj interface{}) {
	...
	if o, ok := obj.(client.Object); ok {
		...
		log.Error("OnAdd missing Object",
			zap.Any("object", obj), zap.String("type", fmt.Sprintf("%T", obj)))
		return
	}
...
func (e EventHandler) OnUpdate(oldObj, newObj interface{}) {
	...
	if o, ok := oldObj.(client.Object); ok {
		...
		log.Error("OnUpdate missing ObjectOld",
			zap.Any("object", oldObj), zap.String("type", fmt.Sprintf("%T", oldObj)))
		return
	}
	...
	if o, ok := newObj.(client.Object); ok {
		...
		log.Error("OnUpdate missing ObjectNew",
			zap.Any("object", newObj), zap.String("type", fmt.Sprintf("%T", newObj)))
		return
	}
...
func (e EventHandler) OnDelete(obj interface{}) {
	...
	if _, ok = obj.(client.Object); !ok {
		...
		if !ok {
			log.Error("Error decoding objects.  Expected cache.DeletedFinalStateUnknown",
				zap.Any("object", obj), zap.String("type", fmt.Sprintf("%T", obj)))
			return
		}
	...
	if o, ok := obj.(client.Object); ok {
		...
		log.Error("OnDelete missing Object",
			zap.Any("object", obj), zap.String("type", fmt.Sprintf("%T", obj)))
		return
	}
