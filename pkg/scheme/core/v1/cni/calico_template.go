...
spec:
 ...
 template:
   ...
   spec:
     ...
     initContainers:
       - name: upgrade-ipam
         image: {{with .CNI.ImageRegistry}}{{.}}/{{end}}calico/cni:{{.CNI.Version}}
       ...
       - name: install-cni
         ...
         securityContext:
           privileged: true
       - name: flexvol-driver
         image: {{with .CNI.ImageRegistry }}{{.}}/{{end}}calico/pod2daemon-flexvol:{{.CNI.Version}}
         volumeMounts:
     ...
     containers:
       - name: calico-node
         image: {{with .CNI.ImageRegistry}}{{.}}/{{end}}calico/node:{{.CNI.Version}}
         env:
           - name: DATASTORE_TYPE
             value: "kubernetes"
...
spec:
 ...
 template:
   ...
   spec:
     ...
     containers:
       - name: calico-kube-controllers
         image: {{with .CNI.ImageRegistry}}{{.}}/{{end}}calico/kube-controllers:{{.CNI.Version}}
         env:
...
// https://projectcalico.docs.tigera.io/archive/v3.16/manifests/calico.yaml
const calicoV31610 = `---
kind: ConfigMap
apiVersion: v1
metadata:
...
const calicoV3261 = `installation:
  registry: {{with .CNI.ImageRegistry}}{{.}}{{end}}
...
tigeraOperator:
  ...
  version: v1.30.4
  registry: {{with .CNI.ImageRegistry}}{{.}}{{else}}quay.io{{end}}
calicoctl:
  image: {{with .CNI.ImageRegistry}}{{.}}{{else}}docker.io{{end}}/calico/ctl
  tag: v3.26.1`
...
tigeraOperator:
  ...
  registry: {{with .CNI.ImageRegistry}}{{.}}{{else}}quay.io{{end}}
calicoctl:
  image: {{with .CNI.ImageRegistry}}{{.}}{{else}}quay.io{{end}}/calico/ctl
  tag: v3.31.5
