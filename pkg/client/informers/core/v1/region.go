...
type RegionInformer interface {
	...
	Lister() corev1lister.RegionLister
}
...
func NewRegionInformer(client clientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
func (f *regionInformer) defaultInformer(client clientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *regionInformer) Lister() corev1lister.RegionLister {
	return corev1lister.NewRegionLister(f.Informer().GetIndexer())
}
