...
func afterEachDeleteRegistries(ctx context.Context, f *framework.Framework, times int) {
	f.AddAfterEach(fmt.Sprintf("delete registries %d time", times), func(f *framework.Framework, failed bool) {
		registries, err := f.KcClient().ListRegistries(ctx, kc.Queries{})
